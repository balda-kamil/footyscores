...
export default function Home() {
  ...
  return (
    <>
      {/* Header */}
      <header className="flex items-center gap-4 px-7 h-14 border-b border-line bg-surface sticky top-0 z-50">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="px-7 pt-6">
          ...
          {/* Stats bar */}
          {loadState === 'loaded' && (
            <div className="flex gap-px bg-line border border-line rounded-lg overflow-hidden mt-5">
              {(
                [
                  ...
                  { label: 'Group Stage',          value: mockMatches.filter((m) => m.stage === 'group').length,  color: '' },
                  { label: 'Knockout',             value: mockMatches.filter((m) => m.stage !== 'group').length,  color: 'text-orange' },
                  { label: 'Endpoints Generated',  value: endpointsVisible ? mockMatches.length : 0,             color: 'text-green' },
                ] as const
              ).map(({ label, value, color }) => (
                <div key={label} className="flex-1 bg-surface px-5 py-[14px] flex flex-col gap-[3px]">
  ...
  return (
    ...
    >
      <div className="bg-surface border border-line-hi rounded-xl w-[680px] max-w-full max-h-[85vh] flex flex-col anim-slide-up shadow-[0_24px_64px_rgba(0,0,0,0.5)]">
        {/* Modal header */}
        <div className="px-[22px] py-[18px] border-b border-line flex items-center gap-3">
        ...
        <div className="p-[22px] overflow-y-auto flex flex-col gap-5">
          ...
          <div>
            ...
            <div className="bg-surface-2 border border-line rounded-[5px] p-[12px_14px] font-mono text-[12px] text-green break-all leading-relaxed relative">
